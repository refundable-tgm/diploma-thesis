...
\end{code}
\captionof{listing}{Funktion für das Managment der \textit{Cookies} und \textit{History} des Browsers}~\\
...
\end{code}
\captionof{listing}{Anzeigen der Komponenten im \textit{Manager}}~\\
...
\textit{v-bind}-Attribute binden Daten von der übergeordneten Komponente in die eingebundene Komponente ein. Bei dem Beispiel \textit{v-bind:cookieset=''cookies''} wird an die Login-Komponente ein Attribut \textit{cookieset} mit dem Wert von der Variable \textit{cookies} mitgegeben.
\newpage
...
\end{code}
\captionof{listing}{Auffangen übergebener Variablen}~\\
...
\end{code}
\captionof{listing}{Route für den \textit{Antrag-Viewer}}~\\
Es wird die Home-Komponente (der \textit{Manager}) aufgerufen, da dieser das Verarbeiten der URL und das Weiterleiten auf die Login- und Antragsansicht übernimmt.
\newpage
...
\end{code}
\captionof{listing}{Navigation auf die zuletzt besuchte Seite}~\\
...
axios.post(this.url + "/login", {	// Sendet ein POST-Abfrage an das Backend
	...
	// Es werden die Daten aus der Abfrage an den \textit{Manager} gesendet, damit eine valide Sitzung erstellt wird.
})
\end{code}
...
\end{code}
\captionof{listing}{\textit{JSON}-Objekt einer Schulveranstaltung}~\\
\newpage
...
Das \textit{JSON}-Objekt wird, wie in Kapitel \hyperref[code_submit_data]{7.3.4.1} beschrieben, erstellt. Für eine Fortbildung gibt es nur wenige Änderungen.
...
\end{code}
\captionof{listing}{\textit{JSON}-Objekt einer Fortbildung}~\\
\newpage
...
Das \textit{JSON}-Objekt wird so erstellt, wie in Kapitel \hyperref[code_submit_data]{7.3.4.1} beschrieben. Für einen \textit{Sonstiges}-Antrag gibt es nur wenige Änderungen.
...
\end{code}
\captionof{listing}{\textit{JSON}-Objekt eines \textit{Sonstiges}-Antrags}~\\
\newpage
...
\end{code}
\captionof{listing}{PDF erstellen und öffnen}~\\
...
\end{code}
\captionof{listing}{Anfrage zum Setzen der Rechte eines Benutzers}~\\